...
class Blocks {
	...
	function cgb_editor_assets() { // phpcs:ignore
		...
		$js_deps = apply_filters( 'wc_ei_block_js_deps', array( 'wp-block-editor', 'wp-blocks', 'wp-i18n', 'wp-element', 'wp-data', 'wp-compose', 'wp-components' ) );

		wp_register_script(
			'wc-ei-block-editor', // Handle.
			plugins_url( '/dist/blocks.build.js', dirname( __FILE__ ) ),
			$js_deps,
			WC_EMAIL_INQUIRY_VERSION,
			array(
				'strategy'  => 'defer',
				'in_footer' => true,
			)
